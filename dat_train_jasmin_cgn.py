...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        # gradient reversal
        domain_feats = GRL.apply(enc_out)
        # domain output
        domain_logits = self.modules.domain(domain_feats)
        p_domain = domain_logits
        ...
        return p_ctc, p_seq, wav_lens, hyps, p_domain

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss (CTC+NLL) given predictions and targets."""

        (p_ctc, p_seq, wav_lens, hyps, p_domain) = predictions
        ...
        for index, ac in enumerate(batch.accent):
            batch.accent[index] = int(batch.accent[index])
        batch_accent = F.one_hot(torch.LongTensor(batch.accent), num_classes=2).to("cuda")
        loss_domain = torch.nn.CrossEntropyLoss()(p_domain, batch_accent)

        loss = loss_domain + loss
...
def dataio_prepare(hparams):
    ...
    sb.dataio.dataset.set_output_keys(
        datasets, ["id", "sig", "wrd", "tokens_bos", "tokens_eos", "tokens", "accent"],
    )
...
if __name__ == "__main__":
   ...
   # )
    ...
    rnn_dim = 768
    domain_classifier = nn.Sequential(nn.Linear(rnn_dim, rnn_dim), nn.LayerNorm(rnn_dim), nn.GELU(), nn.Dropout(0.1),
    ...
    hparams["modules"]["domain"] = domain_classifier
